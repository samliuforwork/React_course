...
function HooksIntersectionExample(){
  var {node, isEntry} = useIntersectionObserver();
  ...
  return (
    <section
      className={className}
      data-name="HooksIntersectionExample" ref={node}>
      <h1>{isEntry ? 'Enter' : 'hidden'}</h1>
    </section>
  )
}
